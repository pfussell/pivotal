...
class simple_credentials_manager : public Botan::Credentials_Manager
{
public:
	simple_credentials_manager(Botan::RandomNumberGenerator& rng) : rng(rng)
	{
		m_certstores.push_back(new Botan::Certificate_Store_In_Memory("/Users/thomas/Development/pivotal/certificates"));
	}

	std::string srp_identifier(const std::string& type, const std::string& hostname)
	{
	...
	bool attempt_srp(const std::string& type, const std::string& hostname)
	{
	...
	std::vector<Botan::Certificate_Store*> trusted_certificate_authorities(const std::string& type, const std::string& /*hostname*/)
	{
	...
	void verify_certificate_chain(const std::string& type, const std::string& purported_hostname,
		const std::vector<Botan::X509_Certificate>& cert_chain)
	{
		try
		{
			Botan::Credentials_Manager::verify_certificate_chain(type, purported_hostname, cert_chain);
		}
	...
	std::string srp_password(const std::string& type, const std::string& hostname, const std::string& identifier)
	{
		...
		bool generate_fake_on_unknown)
	{
		...
		if(pass == "")
		{
			...
			pass.resize(16);
			rng.randomize(reinterpret_cast<Botan::byte*>(&pass[0]), pass.size());
		}
